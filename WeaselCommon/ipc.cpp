 ...
 pipe::~pipe()
{
 ...
 inline orch::orch(std::wstring pipe_name) :
 ...
 inline void orch::start(const callback& cb)
 {
   if (running_) return;
   run(cb);
 ...
 inline void orch::run(const callback& cb)
 {
   ...
   while (true)
   {
     ...
     auto& i = this->pipes_.emplace_back(h_pipe, ov, this->h_terminate_, cb);
     i.run();
   }
 }

 inline void orch::stop()
 {
   SetEvent(h_terminate_);
