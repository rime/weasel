...
int ServerImpl::Run()
{
  ...
  */
  pipeThread = std::make_unique<boost::thread>([this]() {
    this->orch.start(
 [this](pbuffer in, pbuffer out, bool& should_run)
    {
      std::wcout << reinterpret_cast<wchar_t>(in->data()) << std::endl;
      HandlePipeMessage(in, out);
      std::wcout << reinterpret_cast<wchar_t>(out->data()) << std::endl;
    }
    );
...
void ServerImpl::write_send_buffer(std::wstring& msg)
{
  out->write_bytes(reinterpret_cast<const char*>(msg.c_str()), msg.length() * 2);
  //wchar_t* target = reinterpret_cast<wchar_t*>(send_buffer);
  //StrCpyW(target, msg.c_str());
  //target += msg.length();
  //send_buffer = reinterpret_cast<char*>(target);
}
...
DWORD ServerImpl::OnStartSession(WEASEL_IPC_COMMAND uMsg, DWORD wParam, DWORD lParam)
{
  ...
  return m_pRequestHandler->AddSession(
    reinterpret_cast<LPWSTR>(recv_buffer),
    [this](std::wstring& msg) -> bool {
      write_send_buffer(msg);
      //*channel << msg;
      return true;
    }
  );
...
void ServerImpl::HandlePipeMessage(pbuffer in, pbuffer out)
{
  DWORD result;

  PipeMessage* pMsg = reinterpret_cast<PipeMessage*>(in->data());
  PipeMessage pipe_msg = *pMsg;
  out->clear();
  out->write_bytes(reinterpret_cast<char*>(&result), sizeof(DWORD) / sizeof(char));
  this->out = out;
  recv_buffer = in->data() + sizeof(PipeMessage);
  send_buffer = out->data() + sizeof(DWORD);
  test_buf = reinterpret_cast<wchar_t*>(send_buffer);
  ...
  DWORD* pRes = reinterpret_cast<DWORD*>(out->data());
  *pRes = result;
  // resp(result);
}
