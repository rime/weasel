...
LRESULT DictManagementDialog::OnBackup(WORD, WORD code, HWND, BOOL&) {
	...
	if (sel < 0 || sel >= user_dict_list_.GetCount()) {
		...
		MSG_BY_IDS(IDS_STR_SEL_EXPORT_DICT_NAME, IDS_STR_SAD, MB_OK | MB_ICONINFORMATION);
		return 0;
	}
	std::wstring path;
	{
...
LRESULT DictManagementDialog::OnRestore(WORD, WORD code, HWND, BOOL&) {
	...
	if (IDOK == dlg.DoModal()) {
		...
		if (!api_->restore_user_dict(path)) {
			...
			MSG_BY_IDS(IDS_STR_ERR_SUCCESS, IDS_STR_HAPPY, MB_OK | MB_ICONINFORMATION);
		}
	}
	return 0;
}
...
LRESULT DictManagementDialog::OnExport(WORD, WORD code, HWND, BOOL&) {
	...
	if (IDOK == dlg.DoModal()) {
		...
		if (result < 0) {
			...
			MSG_BY_IDS(IDS_STR_ERR_EXPORT_FILE_LOST, IDS_STR_SAD, MB_OK | MB_ICONERROR);
		}
		else {
...
LRESULT DictManagementDialog::OnImport(WORD, WORD code, HWND, BOOL&) {
	...
	if (sel < 0 || sel >= user_dict_list_.GetCount()) {
		...
		MSG_BY_IDS(IDS_STR_SEL_IMPORT_DICT_NAME, IDS_STR_SAD, MB_OK | MB_ICONINFORMATION);
		return 0;
	}
	WCHAR dict_name[MAX_PATH] = {0};
	user_dict_list_.GetText(sel, dict_name);
